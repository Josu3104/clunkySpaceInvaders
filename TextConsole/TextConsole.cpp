...
void TextConsole::writeString(const char *str)
{
   ...
   // postRepaintEvent(); La funcion no es necesaria aqui
}
...
/*
 bool running = true;
        ...
        std::thread cthread(&CClient::run, &cl);
        uint32_t lastFrameTime = getMillis();
    //Cambios empiezan aqui
        while (running) {
            SDL_PollEvent(&e);  // Uso de la funcion Pol_Event para evitar congelamiento
            uint32_t currentTime = getMillis();
            uint32_t frameDuration = currentTime - lastFrameTime;

            //Redraw por intervalos en lugar de tras cada char impreso
            if (needsRedraw && frameDuration > 16) {
                redraw();
                needsRedraw = false;
